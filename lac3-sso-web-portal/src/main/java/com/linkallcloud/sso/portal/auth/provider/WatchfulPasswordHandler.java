...
public abstract class WatchfulPasswordHandler implements PasswordHandler {
	...
	/**
	 * The number of failed login attempts to allow before locking out the source IP
	 * address. (Note that failed login attempts "expire" regularly.)
	 */
	...
	/**
	 * Returns true if the given request comes from an IP address whose allotment of
	 * failed login attemps is within reasonable bounds; false otherwise. Note: We
	 * don't actually validate the user and password; this functionality must be
	...
	/**
	 * Removes one failure record from each offender; if any offender's resulting
	 * total is zero, remove it from the list.
	 */
