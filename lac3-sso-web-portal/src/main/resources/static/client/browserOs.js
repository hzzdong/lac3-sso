        ...
        if(browserOsInfo.mobile){
        	...
        	if(wechatInfo){
        		...
        		if(wechatInfo.length >= 2){
        			browserOsInfo.browserVersion = wechatInfo[1];
        		}
        	}
        	
        	//若是手机，补充手机其他信息
        	var md = new MobileDetect(navigator.userAgent);
        	browserOsInfo.mobileBrand = md.phone();
        	browserOsInfo.os = md.os();
        	if(browserOsInfo.os == "iOS"){
        		browserOsInfo.osVersion = md.version("iOS");
        	} else if(browserOsInfo.os == "AndroidOS"){
        		browserOsInfo.osVersion = md.version("Android");
        	} 
        	browserOsInfo.browser = md.userAgent();
        	browserOsInfo.browserVersion = md.versionStr('Build');
        	browserOsInfo.model = md.mobile();
        	browserOsInfo.tablet = md.tablet();
