...
public class AccountKiss extends BaseKiss {
	...
	public Account fetchByAccount(Trace t, String account) {
		...
		String responseJson = HttpClientFactory.me(false).post(getUmOapiUrl() + "/face/Account/fetchByAccount",
				sendMsgPkg);
	...
	public Account fetchByMobile(Trace t, String mobile) {
		...
		String responseJson = HttpClientFactory.me(false).post(getUmOapiUrl() + "/face/Account/fetchByMobile",
				sendMsgPkg);
		Account result = unpackMessage(responseJson, new TypeReference<ObjectFaceResponse<Account>>() {
		});
		return result;
	...
	public Account loginValidate(Trace t, AccountValidateRequest req) {
		...
		String responseJson = HttpClientFactory.me(false).post(getUmOapiUrl() + "/face/Account/loginValidate",
				sendMsgPkg);
		Account result = unpackMessage(responseJson, new TypeReference<ObjectFaceResponse<Account>>() {
		});
		return result;
	...
	public Boolean modifyPassword(Trace t, ModifyPasswordRequest req) {
		String sendMsgPkg = packMessage(t, req);
		String responseJson = HttpClientFactory.me(false).post(getUmOapiUrl() + "/face/Account/modifyPassword",
				sendMsgPkg);
