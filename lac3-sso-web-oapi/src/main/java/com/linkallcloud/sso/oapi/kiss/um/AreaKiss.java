...
public class AreaKiss extends BaseKiss {
	...
	public List<Tree> getTreeNodes(Trace t, Boolean valid) {
		...
		String responseJson = HttpClientFactory.me(false).post(getUmOapiUrl() + "/face/Area/getTreeNodes", sendMsgPkg);
	...
	public Area fetchById(Trace t, Long id) {
		...
		String responseJson = HttpClientFactory.me(false).post(getUmOapiUrl() + "/face/Area/fetchById", sendMsgPkg);
	...
	public Area fetchByGovCode(Trace t, String govCode) {
		...
		String responseJson = HttpClientFactory.me(false).post(getUmOapiUrl() + "/face/Area/fetchByGovCode", sendMsgPkg);
		Area result = unpackMessage(responseJson, new TypeReference<ObjectFaceResponse<Area>>() {
		});
		return result;
	}
	...
	public List<Area> find(Trace t, ListFaceRequest req) {
		...
		String responseJson = HttpClientFactory.me(false).post(getUmOapiUrl() + "/face/Area/find", sendMsgPkg);
	...
	public List<Area> findChildren(Trace t, ParentCodeAreaRequest req) {
		String sendMsgPkg = packMessage(t, req);
		String responseJson = HttpClientFactory.me(false).post(getUmOapiUrl() + "/face/Area/findChildren", sendMsgPkg);
		List<Area> result = unpackMessage(responseJson, new TypeReference<ObjectFaceResponse<List<Area>>>() {
		});
		return result;
	}
