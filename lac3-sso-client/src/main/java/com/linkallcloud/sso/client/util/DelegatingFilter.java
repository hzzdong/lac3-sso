 ...
 *
 * cn.zj.pubinfo.sso.client.util.DelegatingFilter.java 
 *
 * 2011-6-14
 * 
 */
...
/**
 * A Delegating Filter looks up a parameter in the request object and matches (either exact or using Regular
 * Expressions) the value. If there is a match, the associated filter is executed. Otherwise, the normal chain is
 * executed.
 * 
 * 2011-6-15
 * 
...
public final class DelegatingFilter implements Filter {
     ...
     */
    private final Map<String, Object> delegators;
    ...
    /**
     * Whether the key in the delegators map is an exact match or a regular expression.
     */
    private final boolean exactMatch;

    public DelegatingFilter(final String requestParameterName, final Map<String, Object> delegators,
    ...
    public DelegatingFilter(final String requestParameterName, final Map<String, Object> delegators,
        ...
        for (final Iterator<String> iter = delegators.keySet().iterator(); iter.hasNext();) {
    ...
    public void doFilter(final ServletRequest request, final ServletResponse response, final FilterChain filterChain)
        ...
        final String parameter = request.getParameter(this.requestParameterName);

        if (CommonUtils.isNotEmpty(parameter)) {
            for (final Iterator<String> iter = this.delegators.keySet().iterator(); iter.hasNext();) {
