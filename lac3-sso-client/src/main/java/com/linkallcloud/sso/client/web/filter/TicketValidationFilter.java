...
/**
 * Implementation of a Filter that checks for a "ticket" and if one is found, will attempt to validate the ticket. On a
 * successful validation, it sets the Assertion object into the session. On an unsuccessful validation attempt, it sets
 * the response code to 403.
 * <p/>
 * This filter needs to be configured after the authentication filter (if that filter exists in the chain).
 * 
 * 2011-6-14
 * 
...
public final class TicketValidationFilter extends AbstractSSOFilter {
    ...
    /**
     * Specify whether the filter should redirect the user agent after a successful validation to remove the ticket
    ...
    /**
     * Constructor that takes the severName (or serviceUrl) and the TicketValidator. Either serveName or serviceUrl is
     * required (but not both).
     * 
     * @param siteCode
     * @param serverName
     *            the name of the server in <hostname>:<port> combination, if using a non-standard port.
     * @param serviceUrl
     *            the url to always redirect to.
     * @param ticketValidator
     *            the validator to validate the tickets.
     * @param excludePattern
     */
    public TicketValidationFilter(final String siteCode, final String serverName, final String serviceUrl,
            final TicketValidator ticketValidator, final UrlPattern excludePattern) {
        this(siteCode, serverName, serviceUrl, true, ticketValidator, false, excludePattern);
    }

    /**
     * Constructor that takes the severName (or serviceUrl), TicketValidator, useSession and redirectAfterValidation.
     ...
     *            the url to always redirect to.
     * @param useSession
     *            flag to set whether to store stuff in the session.
     * @param ticketValidator
     *            the validator to validate the tickets.
     * @param redirectAfterValidation
     *            whether to redirect to remove the ticket.
     * @param excludePattern
     */
    public TicketValidationFilter(final String siteCode, final String serverName, final String serviceUrl,
            final boolean useSession, final TicketValidator ticketValidator, final boolean redirectAfterValidation,
            final UrlPattern excludePattern) {
        ...
        if (CommonUtils.isNotBlank(ticket)) {
            ...
            try {
                final Assertion assertion =
                        this.ticketValidator.validate(ticket, new SimpleService(constructServiceUrl(request, response),
                                this.siteCode));
